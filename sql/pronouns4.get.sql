...
WHERE (LOWER(languages.name) = LOWER($1) OR LOWER(languages.iso_639_1) = LOWER($1) OR LOWER(languages.iso_639_3) = LOWER($1))
  AND (LOWER($2) = ANY(pronouns.cases) OR $2 = ANY(pronouns.cases))
  AND (LOWER($3) = ANY(pronouns.cases) OR $3 = ANY(pronouns.cases))
  AND (LOWER($4) = ANY(pronouns.cases) OR $4 = ANY(pronouns.cases))
  AND (LOWER($5) = ANY(pronouns.cases) OR $5 = ANY(pronouns.cases))
;
